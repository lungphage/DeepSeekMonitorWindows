...
其中包含 API Key 和用量 Token。自 v1.2.0 起，这两项凭据使用 Windows DPAPI 加密存储（绑定当前 Windows 用户，换机或换用户无法解密），旧版明文配置会在首次读取时自动迁移为加密格式。**仍请不要提交该文件，也不要把截图、日志或配置文件中的密钥内容公开。**
...
- tauri-plugin-notification，余额/预算提醒的系统通知
- reqwest 0.12，启用 json
- serde
- serde_json
- log
- base64
- windows-sys，DPAPI 凭据加密
...
- 动态模型支持：用量接口返回的所有模型都会展示，不再只硬编码 V4 Flash / V4 Pro，新模型自动配色。
- 托盘悬停提示：鼠标悬停托盘图标即可查看余额、今日消耗与本月消费，无需打开窗口。
- 提醒与预算：可设置余额提醒阈值和月度预算，触发时发送 Windows 系统通知；余额卡显示预算使用进度条。
- 余额可用天数预测：按近 7 日日均消耗估算余额还能用多少天，并显示充值/赠金余额拆分。
- 当日消耗环比：显示与昨日相比的涨跌百分比。
- 历史月份查看：详情页可切换任意月份，支持近 7 天 / 全月视图，以及按日 Token / 按日费用两种指标。
- CSV 导出：详情页一键导出当月逐日逐模型用量明细到下载文件夹（带 BOM，Excel 可直接打开）。
- 窗口置顶（钉住）按钮、Esc 隐藏窗口、可选的失焦自动隐藏。
- 主题三态：深色 / 浅色 / 跟随系统。

修复与安全加固：

- 修复自动刷新时整个界面闪烁重置为"查询中"的问题，刷新期间保留旧数据，刷新按钮带旋转反馈。
- 修复开机自启注册表路径未加引号的问题（安装在含空格路径时自启可能失效，且存在劫持风险）。
- 修复切换开机自启时闪现控制台黑框的问题。
- API Key 与用量 Token 改用 Windows DPAPI 加密存储（绑定当前用户），旧版明文配置自动迁移。
- 收紧 Tauri capability：远程登录窗口不再拥有任何 IPC 权限，token 仅经窗口标题通道回传且读取后立即复原标题；配置生产 CSP。
- 修复余额接口多币种时只取第一条的问题，现优先取 CNY。
- 按北京时间切分"今天"，修复非东八区用户当日数据错位。
- 并发请求加序号守卫，慢响应不再覆盖新数据。
